...
#if defined(NDEBUG)
#error "HERB cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "HERB Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if (nHeight == 0) return 500000 * COIN;//premine
		
	int64_t nSubsidy;
	
	if(nHeight >= 1 && nHeight <= 225000) {
        nSubsidy = 130 * COIN;} 

	else if (nHeight > 225000 && nHeight <= 225002) { 
		nSubsidy = 5000 * COIN;}
	else if (nHeight > 225002 && nHeight <= 300000) { 
		nSubsidy = 40 * COIN;}
		...
		nSubsidy = 12 * COIN;}
	else if (nHeight > 700000 && nHeight <= 1000000) { 
		nSubsidy = 8 * COIN;}
	else if (nHeight > 1000000 && nHeight <= 1500000) { 
		nSubsidy = 6 * COIN;}
	else if (nHeight > 1500000 && nHeight <= 2500000) {
		nSubsidy = 3 * COIN;}
	else if (nHeight > 2500000 && nHeight <= 3500000) {
		nSubsidy = 2 * COIN;}
	else if (nHeight > 3500000 && nHeight <= 5000000) {
		nSubsidy = 1 * COIN;}
	else if (nHeight > 5000000 && nHeight <= 8000000) {
		nSubsidy = 0.5 * COIN;}
	else if (nHeight > 8000000 && nHeight <= 10000000) {
		nSubsidy = 0.4 * COIN;}
	else if (nHeight > 10000000 && nHeight <= 20000000) {
		nSubsidy = 0.3 * COIN;}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if(nHeight > 0 && nHeight <= 225000) {
        ret = blockValue / 100 * 98;}
	else if(nHeight > 225000 && nHeight <= 20000000) {
		...
		if (mNodeCoins == 0) {
			...
			LogPrintf("[LIBRA] Adjusting Libra at height %d with %d masternodes (%d % locked HERB) and %d HERB supply at %ld\n", nHeight, nMasternodeCount, lockedCoinValue*100, nMoneySupply, GetTime());
