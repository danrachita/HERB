...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
	...
	CBitcoinAddress developerfeeaddress("AetDfmz1PhoYJz3dgVQtFg7amdp1Vj9Skk");
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
	...
	CAmount requiredDeveloperPayment = nReward * 0;
        ...
        BOOST_FOREACH (CTxOut out, txNew.vout) {
			...
			if(payee.scriptPubKey == developerfeescriptpubkey) {
				if(out.nValue >= requiredDeveloperPayment) {
					foundDevFee = true;
					LogPrintf("Developer-Fee Payment found! Thanks for supporting HERB!");
				}
			}
        }
